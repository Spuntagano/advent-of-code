...
fn step(seats: &mut Vec<Vec<SeatStatus>>) -> bool {
    ...
    for (row_index, row) in initial_seats.iter().enumerate() {
        for (column_index, _) in row.iter().enumerate() {
            ...
            } else if (*seats)[row_index][column_index] == SeatStatus::Occupied && occupied_seat_count >= 5 {
...
fn count_adjecant_seats(initial_seats: &Vec<Vec<SeatStatus>>, row_index: i32, column_index: i32) -> i32 {
    ...
    for i in -1..=1 as i32 {
        for j in -1..=1 as i32 {
            ...
            if check_line(initial_seats, row_index, column_index, i, j) {
...
fn check_line(initial_seats: &Vec<Vec<SeatStatus>>, row_index: i32, column_index: i32, row_diff: i32, column_diff: i32) -> bool {
    let mut check_row_index = row_index + row_diff;
    let mut check_column_index = column_index + column_diff;
    let mut occupied_seat_found = false;
    let mut empty_seat_found = false;
    while
        !empty_seat_found &&
        !occupied_seat_found &&
        check_row_index >= 0 &&
        ...
        check_column_index < initial_seats[0].len() as i32 {
            match initial_seats[check_row_index as usize][check_column_index as usize] {
                SeatStatus::Empty => empty_seat_found = true,
                SeatStatus::Occupied => occupied_seat_found = true,
                SeatStatus::Floor => (),
            };
            check_row_index += row_diff;
            check_column_index += column_diff;
    }
