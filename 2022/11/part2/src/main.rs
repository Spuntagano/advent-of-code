...
fn get_input() -> Vec<Monkey> {
    ...
    monkeys.push(Monkey {
        item_inspected: 0,
        items: vec![57],
        operation:|value: &u64| -> u64 {
            return (value * 13) % 9699690;
        },
        test: |value: &u64| -> u64 {
            if value % 11 == 0 {
                return 3;
...
fn parse_file(_contents: String) -> Result<i64, ParsingError> {
    ...
    for _ in 0..10000 {
        for monkey_index in 0..monkeys.len() {
            ...
            for item_index in 0..monkey.items.len() {
                ...
                let value = *&monkey.operation.to_owned()(item);
    ...
    let mut items_inspected_by_monkey = monkeys.iter().map(|val| val.item_inspected).collect::<Vec<u64>>();
    items_inspected_by_monkey.sort();
    items_inspected_by_monkey.reverse();

    return Ok(items_inspected_by_monkey[0] as i64 * items_inspected_by_monkey[1] as i64);
}
